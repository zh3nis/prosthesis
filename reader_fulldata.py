...
class MyData:
  def __init__(self, path, set_indices, config, data_type):
    ...
    # for testing 2, 7, 8, 10
    self.subjects = ["almaskhan", "ainur", "akmaral", "aikorkem", "artemiy", "bauka", "beksultan", "kazbek", "madina", "olzhas", "sultanmurat", "zhansaya"]
  ...
  def read_data(self):
    ...
    for i in self.set_indices:
      drname1 = '/mnt/data/%s/day1/' % self.subjects[i]
      drname2 = '/mnt/data/%s/day2/' % self.subjects[i]
      day1dirs = [f for f in os.listdir(drname1) if os.path.isdir(os.path.join(drname1, f))]
      day2dirs = [f for f in os.listdir(drname2) if os.path.isdir(os.path.join(drname2, f))]
      buf1 = '/mnt/data/%s/day1/' % self.subjects[i]
      buf2 = '/mnt/data/%s/day2/' % self.subjects[i]
      print(i, end=' .. ')
      sys.stdout.flush()
      fulldirlist = []
      ...
      for mydir in day2dirs:
        depthdir = "{0}{1}/".format(buf2, mydir)
  	fulldirlist.append(depthdir)
      ...
      for mydir in fulldirlist:
        ...
        annf = os.path.join(buf, 'annotations.txt')
	  ...
	  #print(x[j])
          str_action, str_startInd, str_endInd = re.split('\s+', x[j])
          label, startInd, endInd = self.str_to_label[str_action.lower()], int(str_startInd), int(str_endInd)
